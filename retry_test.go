...
func TestRetryNonStream_DeadlineContextStopsImmediately(t *testing.T) {
	p := &Portal{
		session: session.New(),
		logger:  logger.NewNopLogger(),
	}
	...
	if channelCalls != 0 {
		t.Fatalf("超时上下文不应拉取通道，实际调用次数：%d", channelCalls)
	}
}

func TestRetryNonStream_ContextCanceledWithoutClientCtxShouldBeServerCancel(t *testing.T) {
	p := &Portal{
		session: session.New(),
		logger:  logger.NewNopLogger(),
	}
